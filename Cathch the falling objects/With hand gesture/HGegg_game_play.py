...
class Newton:
    def __init__(self):
        self.width = basket_images[0].get_width()
        self.height = basket_images[0].get_height()
        self.x = (WINDOW_WIDTH - self.width) // 2
        self.y = WINDOW_HEIGHT - self.height - 10
        self.speed = NEWTON_SPEED
        self.eggs_caught = 0
        self.current_image = basket_images[0]
    ...
    def catch_egg(self):
        self.eggs_caught += 1
        # Determine which image to show based on number of eggs caught
        if self.eggs_caught <= 4:
            self.current_image = basket_images[self.eggs_caught]
        else:
            # Alternate between images 3 and 4 for eggs beyond 4
            if self.eggs_caught % 2 == 1:  # odd number (5th, 7th, etc.)
                self.current_image = basket_images[3]
            else:  # even number (6th, 8th, etc.)
                self.current_image = basket_images[4]
