...
GOLD = (255, 215, 0)  # Gold color for high score
...
class Dolphin:
    ...
    def move(self, direction):
        ...
        if direction == 'down':
            # Move dolphin down if not jumping and not too low
            if not self.is_jumping and self.y < WINDOW_HEIGHT - self.height - 10:
                self.y += self.speed
                self.original_y = self.y  # Update original y for bobbing
    ...
    def update_animation(self):
        # Only update animation when moving or jumping
        keys = pygame.key.get_pressed()
        is_moving = keys[K_LEFT] or keys[K_RIGHT] or keys[K_UP] or keys[K_DOWN] or self.is_jumping
    ...
    def update(self):
        # Update jump first, as it may affect other animations
        just_landed = self.update_jump()
        self.update_animation()
        self.update_bobbing()
        
        # Return False instead of just_landed to never trigger splash creation
        return False
...
def game_over_screen(score, high_score):
    ...
    # Draw high score
    if new_record:
        high_score_text = final_score_font.render("NEW HIGH SCORE!", True, GOLD)
    else:
        high_score_text = final_score_font.render(f"HIGH SCORE: {high_score}", True, GOLD)
    window.blit(high_score_text, (WINDOW_WIDTH//2 - high_score_text.get_width()//2, WINDOW_HEIGHT//2 + 20))
    
    # Draw instructions
    show_text("Press SPACE to play again", WINDOW_WIDTH//2 - 150, WINDOW_HEIGHT//2 + 70, WHITE)
    show_text("Press ESC to quit", WINDOW_WIDTH//2 - 100, WINDOW_HEIGHT//2 + 120, WHITE)
        ...
        while game_running:
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    ...
                    if event.key == K_DOWN:
                        dolphin.move('down')  # Down key moves dolphin down
            ...
            if keys[K_RIGHT]:
                dolphin.move('right')
            if keys[K_DOWN]:
                dolphin.move('down')  # Allow holding down key to move down continuously
            
            # Update water background
            water_bg.update()
            
            # Update dolphin animation and jump
            # The dolphin.update() now always returns False, so no splash will be created
            dolphin.update()
            ...
            for ball in balls:
                ...
                # Check for collision with dolphin
                if not ball.caught and dolphin.x < ball.x < dolphin.x + dolphin.width and \
                   dolphin.y < ball.y < dolphin.y + dolphin.height:
                    ...
                    # Update high score in real-time if player beats it
                    if score > high_score:
                        high_score = score
            ...
            show_text(f"High Score: {high_score}", WINDOW_WIDTH - 200, 40, GOLD)
            draw_lives(lives)
